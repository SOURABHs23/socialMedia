...
export const toggleFriendship = async (req, res, next) => {
  ...
  if (resp.success) {
    ...
    next(new ApplicationError(resp.error.statusCode, resp.error.msg));
  }
};
...
export const getPendingRequest = async (req, res, next) => {
  const userId = req.userId; // because the user who is logged in can only see its own friend requests
